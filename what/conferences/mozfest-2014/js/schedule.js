function Schedule(options) {
  ...
  schedule.init = function (options) {
    ...
    schedule.$toggles = $('<ul id="schedule-controls">')
      .appendTo("header")
      .hide();
    ...
    if (Modernizr.localstorage) {
      localStorage["mozfest_saved_sessions"] =
        localStorage["mozfest_saved_sessions"] || "";
      schedule.savedSessionIDs =
        _.compact(localStorage["mozfest_saved_sessions"].split(",")) || [];
    }
  ...
  schedule.loadSessions = function (callback) {
    if (schedule.sessionList.length) {
      ...
      $.getJSON(schedule.sourceJSON).done(function (results) {
  ...
  schedule.makeSessionItemTemplateData = function (sessionItem, expanded) {
    var templatedata = {
      ...
      sessionClass: sessionItem.everyone
        ? "everyone"
        : sessionItem.length == "1 hour"
          ? "length-short"
          : "length-long",
      showDay: false,
      showLeaders: false,
  ...
  schedule.addStars = function (containerClass) {
    if (Modernizr.localstorage) {
      $(containerClass + ":not(.session-everyone)").append(
  ...
  schedule.addToggles = function () {
    if (Modernizr.localstorage) {
      ...
      schedule.tabList.splice(schedule.tabList.length - 1, 0, {
        name: "Favorites",
      });
    }
  ...
  schedule.showFullSessionList = function () {
    ...
    _.each(fullList, function (v, k) {
      ...
      schedule.writeSession(
        schedule.$container,
        templateData,
        schedule.sessionListItemTemplate
      );
    });
  ...
  schedule.addListControls = function () {
    ...
    var expand = $(
      '<a id="show-descriptions" data-action="show" href="#"><i class="fa fa-plus-circle"></i> Show descriptions</a>'
    ).appendTo(schedule.$container);
    ...
    $("#list-filter")
      .change(function () {
        ...
        if (filterVal) {
          ...
          var filteredSessions = _.filter(
            schedule.sessionList,
            function (v, k) {
              return (
                v.title.toUpperCase().indexOf(filterVal.toUpperCase()) >= 0 ||
                v.leader.toUpperCase().indexOf(filterVal.toUpperCase()) >= 0 ||
                v.description.toUpperCase().indexOf(filterVal.toUpperCase()) >=
                  0
              );
            }
          );
        ...
        if ($(".session-list-item:visible").length == 0) {
          $("#no-results").remove();
          $("#filter-form").after(
            '<p id="no-results">No matching results found.</p>'
          );
        } else {
          $("#no-results").remove();
        }
  ...
  schedule.showFavorites = function () {
    ...
    schedule.$container
      ...
      .empty()
      .append(
        '<p class="overline">Star sessions to store a list on this device</p>'
      )
      .append(schedule.sessionListTemplate);
  ...
  schedule.addListeners = function () {
    ...
    schedule.$container.on("click", "#show-descriptions", function (e) {
      ...
      if (action == "show") {
        ...
        clicked
          .html('<i class="fa fa-minus-circle"></i> Hide descriptions')
          .data("action", "hide");
      } else {
        $(".session-list-item").find(".session-description").hide();
        clicked
          .html('<i class="fa fa-plus-circle"></i> Show descriptions')
          .data("action", "show");
      }
    });
    ...
    schedule.$container.on("click", ".favorite", function (e) {
      ...
      if (clicked.hasClass("favorite-active")) {
        ...
        schedule.savedSessionIDs = _.without(
          schedule.savedSessionIDs,
          sessionID
        );
        ...
        window.applicationCache.addEventListener(
          ...
          function (e) {
            if (
              ...
              if (
                confirm("A new version of the schedule is available. Load it?")
              ) {
